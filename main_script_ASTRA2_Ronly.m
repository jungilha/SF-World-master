...
for imgIdx = 205:M
    ...
    % image
    % 파일 경로 설정
    filePath = [datasetPath '/color_new/' sprintf('%06d', imgIdx) '.jpg'];
    % 파일 존재 여부 확인
    if exist(filePath, 'file') ~= 2
        % 파일이 존재하지 않으면 경고 메시지를 출력하고 반복문 건너뛰기
        fprintf('Warning: File %s does not exist.\n', filePath);
        continue; % 다음 반복으로 건너뜀
    end
    imageCurForLine = getImgInASTRAdataset(datasetPath, ICLNUIMdataset, cam, imgIdx, 'gray');
    imageCurForMW = getImgInASTRAdataset(datasetPath, ICLNUIMdataset, cam, imgIdx, 'rgb');
    depthCurForMW = getImgInASTRAdataset(datasetPath, ICLNUIMdataset, cam, imgIdx, 'depth');
    lines = readmatrix([datasetPath '/csv/' sprintf('%06d', imgIdx) '_line.csv']);
    ...
    %{
    if 10 <= imgIdx && imgIdx <= 110
    optsLPIC.minSampleRatio = 0.9;
    end
    if 50 <= imgIdx && imgIdx <= 110
    optsLPIC.minSampleRatio = 0.7;
    end
    if 110 <= imgIdx && imgIdx <= 150
    optsLPIC.minSampleRatio = 0.9;
    end
    %}
    ...
    elseif (systemInited_LPIC)
        ...
        [R_cM, R_SLP, vpInfo, pNV, sNV, sPP, clusteredLinesIdx, maxVoteSumIdx] = trackSanFranciscoWorld_2(R_cM, pNV_old,R_SLP, imageCurForLine, imageCurForMW, depthCurForMW,lines, cam, optsLPIC);
        if R_cM == zeros(3,3)
            continue;
        end

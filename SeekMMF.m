function [MF_can, MF, FindMF] = SeekMMF(normVectors,numTrial,ConvergeAngle,ConeAngle,c,minNumSample)
    ...
    %     breakF = 0;
        while acos((trace(R'*R_update) - 1)/2) > ConvergeAngle || iter == 1
            ...
            for a = 1:3
                Ra = [R(:,mod(a+3,3)+1),R(:,mod(a+4,3)+1),R(:,mod(a+5,3)+1)]'; %a=1 ->2,3,1 / a=2->3,2,1 / a=3->1,2,3
                F = [];
            ...
            R = [M(:,directionFound(1)) M(:,directionFound(2)) M(:,6-(directionFound(1)+directionFound(2)))];

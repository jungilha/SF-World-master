...
TUMRGBDdataset.confi.ntime = double(TUMRGBDdataset.confi.time - minReferenceTime);
TUMRGBDdataset.vicon.ntime = double(TUMRGBDdataset.vicon.time  - minReferenceTime);

TUMRGBDdatasetNet.rgb.Ratio = rawTUMRGBDdataset.rgb.Ratio;
TUMRGBDdatasetNet.rgb.dRatio = rawTUMRGBDdataset.rgb.dRatio;
...
% confi part
matchingTimeIdx_confi = zeros(size(TUMRGBDdataset.rgb.ntime,2),2);
for i = 1:size(matchingTimeIdx_confi,1)
    temp_difference = abs( TUMRGBDdataset.confi.ntime - TUMRGBDdataset.rgb.ntime(i) );
    [~,minTimeIdx] = min(temp_difference);
    
    matchingTimeIdx_confi(i,1) = minTimeIdx; % ntime in depth index
    matchingTimeIdx_confi(i,2) = i;               % ntime in rgb index
end

TUMRGBDdataset.rgb.matchingTimeIdx_confi = matchingTimeIdx_confi.';
...
TUMRGBDdatasetNet.rgb.KD = TUMRGBDdataset.rgb.KD;
...
% re-load confi information
for k=1:M
    confiIdx = TUMRGBDdataset.rgb.matchingTimeIdx_confi(1,imInit+k-1);
    
    TUMRGBDdatasetNet.confi.ntime(k) = TUMRGBDdataset.confi.ntime(:,confiIdx);
    TUMRGBDdatasetNet.confi.imgName{k} = TUMRGBDdataset.confi.imgName{confiIdx};
end

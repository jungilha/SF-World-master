...
while (true)
    ...
    lineCloseIdx = (abs(greatcircleNormal(1,:) * greatcircleNormal.') > cos(deg2rad(0.5))).';
...
removeLineIdx_u = find(lines_final(:,1) > (cam.Cols*0.95) & lines_final(:,3) > (cam.Cols*0.95));
removeLineIdx_v = find(lines_final(:,2) > (cam.Rows*0.95) & lines_final(:,4) > (cam.Rows*0.95));

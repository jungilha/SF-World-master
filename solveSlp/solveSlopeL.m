...
function [theta_sol,u_normal, hor_AF] = solveSlopeL(R_cM_initial, planeNormalVector, pNVIdx, greatcircleNormal, alpha)
proximytyAngle2 = [];
proximytyAngle1= [];
syms theta;
colors = {'r', 'g', 'b', 'c', 'm', 'y'};
theta_sol = [];
if(pNVIdx == 3 || pNVIdx == 1)
alpha_found = alpha;
elseif (pNVIdx ==2)
...
if ~isempty(theta_sol_total)
    ...
    figure(20);
    plot3([0 vector1(1)],[0 vector1(2)],[0 vector1(3)], 'Color' ,'k','LineWidth', 3);
    [corrIdx, ~, find_Idx,proximytyAngle1] = identyfyVP_SLP(R_cM_initial, excIdx, vector1);
    if(find_Idx ==0)
        vector1 =[];
    end
    vector2 = rotationMatrix2*-u_normal;
    plot3([0 vector2(1)],[0 vector2(2)],[0 vector2(3)], 'Color' ,'k','LineWidth', 3);
    [corrIdx, ~, find_Idx,proximytyAngle2] = identyfyVP_SLP(R_cM_initial, excIdx, vector2);
    if(find_Idx ==0)
        vector2 = [];
    end
    hor_AF = [vector1 vector2];
    proximytyAngle = [proximytyAngle1 proximytyAngle2];
    [~, hor_Idx] = min(proximytyAngle);
    hor_AF = hor_AF(:,hor_Idx);
else
    hor_AF = [];
end

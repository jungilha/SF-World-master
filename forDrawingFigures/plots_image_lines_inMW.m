function plots_image_lines_inMW(R_cM, vpInfo, imageCurForLine, cam, optsLPRVO)

% assign current parameters
L = optsLPRVO.imagePyramidLevel;
...
for k = 1:vpInfo(1).n
    ...
    plot([linePixelPts(1),linePixelPts(3)],[linePixelPts(2),linePixelPts(4)],'r','LineWidth',3.0);
end
for k = 1:vpInfo(2).n
    ...
    % re-arrange pixel point
    linePixelPts = [u_start, v_start, u_end, v_end] / (2^(L-1));
    plot([linePixelPts(1),linePixelPts(3)],[linePixelPts(2),linePixelPts(4)],'g','LineWidth',3.0);
end
for k = 1:vpInfo(3).n
    ...
    % re-arrange pixel point
    linePixelPts = [u_start, v_start, u_end, v_end] / (2^(L-1));
    plot([linePixelPts(1),linePixelPts(3)],[linePixelPts(2),linePixelPts(4)],'b','LineWidth',3.0);
end

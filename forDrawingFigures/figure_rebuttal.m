...
figure;
plot_plane_image(pNV, sNV, sPP, imageCurForMW, optsLPRVO); hold on;               % AlphaData: 0.50
plots_extended_lines(R_cM, vpInfo, imageCurForLine, cam, optsLPRVO); hold off;   % LineWidth: 3.0
...
% plot 1-plane & multiple lines
figure;
plot_plane_image(pNV, sNV, sPP, imageCurForMW, optsLPRVO); hold on;               % AlphaData: 0.45
plots_extended_lines(R_cM, vpInfo, imageCurForLine, cam, optsLPRVO); hold off;   % LineWidth: 2.0
set(gca,'units','pixels'); x = get(gca,'position');
set(gcf,'units','pixels'); y = get(gcf,'position');
set(gcf,'position',[y(1) y(2) x(3) x(4)]);
set(gca,'units','normalized','position',[0 0 1 1]);

function plots_extended_lines_tracking(R_cM, vpInfo, imageCurForLine, cam, optsLPIC)
...
for k = 1:vpInfo(4).n
    ...
    % re-arrange pixel point
    linePixelPts = [u_start, v_start, u_end, v_end]/ (2^(L-1));
    plot([linePixelPts(1),linePixelPts(3)],[linePixelPts(2),linePixelPts(4)],'c','LineWidth',2.0);
end
for k = 1:vpInfo(5).n
    ...
    % re-arrange pixel point
    linePixelPts = [u_start, v_start, u_end, v_end]/ (2^(L-1));
    plot([linePixelPts(1),linePixelPts(3)],[linePixelPts(2),linePixelPts(4)],'m','LineWidth',2.0);
end

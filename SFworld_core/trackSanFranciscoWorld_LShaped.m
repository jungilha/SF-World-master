function [R_cM_final, R_SLP, vpInfo, planeNormalVector, surfaceNormalVector, surfacePixelPoint, clusteredLinesIdx, maxVoteSumIdx] = trackSanFranciscoWorld_LShaped(R_cM_old, pNV_old,R_SLP_old, imageCurForLine, imageCur, depthCur,lines, cam, optsLPIC ,imgIdx)
...
if (isTracked == 0 || imgIdx == 2442 || imgIdx == 2518 || imgIdx == 2900 || imgIdx == 2920 || imgIdx == 2930) % L 2 
%if (isTracked == 0 || imgIdx == 214 || imgIdx == 389)
...
while (true)
    ...
    [R_cM_new, clusteredLinesIdx,R_SLP, maxVoteSumIdx, ransacIterCnt] = detectOrthogonalLine2RANSAC_LODEP(R_SLP_old, planeNormalVector, alpha, pNVIdx, lines, Kinv, cam, optsLPIC,imageCurForLine);
